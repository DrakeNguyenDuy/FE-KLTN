export const validate = (currentElement, rules, value, selector, fieldName) => {
    let isValidate = true;
    let message = '';
...
const getErrorEmelent = (currentElement, selector) => {
    ...
    let i = 0;
    while (parent === null) {
        parent = currentElement.parentElement.querySelector(selector);
        i++;
        if (i > 20) break;
    }
    return parent;
...
const handleValidateUpdateCV = (data) => {
    ...
    if (!checkDob.isValidate) return checkAddress.message;
